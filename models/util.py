...
class _Upsample(nn.Module):
    # up0=(128,256,128), up1=(128,128,128), up2=(128,64,128)
    def __init__(self, num_maps_in, skip_maps_in, num_maps_out, use_bn=True, k=3):
        ...
        self.bottleneck = _BNReluConv(skip_maps_in, num_maps_in, k=1, batch_norm=use_bn)    # 1x1 conv
        self.blend_conv = _BNReluConv(num_maps_in, num_maps_out, k=k, batch_norm=use_bn)

    def forward(self, x, skip):
        skip = self.bottleneck.forward(skip)
        skip_size = skip.size()[2:4]
        x = upsample(x, skip_size)      # step 1: low to high resolution by bilinearly
        # TODO: SE-like channel-wise spatial attention, but more than 128*128*6=98304 #params
        x = x + skip                    # step 2: ele-wise summed up
        x = self.blend_conv.forward(x)  # step 3: blend conv 3x3
        return x
